...
class Record:
    ...
    @input_error
    def find_in_list(self, phone) -> int | None:
    ...
    def remove_phone(self, phone):  # видалення номеру телефона
        ...
        if i != None:
            del self.phones[i]
            return f'Phone number {phone} was deleted from {self.name.value} contact.'
        else:
...
class AddressBook(UserDict):
    ...
    def birthdays(self) -> list | str:
        ...
        for v in self:
            if not self.data[v].birthday:
                pass
            else:
                birthday = self.data[v].birthday.value
                birthday = birthday.replace(year=current_date.year)  # зміна року народження на поточний
                if birthday.toordinal() < current_date.toordinal():  # якщо дн вже пройшов, додається 1 рік
                    birthday = birthday.replace(year=current_date.year + 1)
                if birthday.toordinal() - current_date.toordinal() <= 7:  # якщо різниця менше 7 днів:
                    if birthday.weekday() == 5:  # якщо день народження випадає на суботу, додається 2 дні
                        birthday = birthday + timedelta(days=2)
                    elif birthday.weekday() == 6:  # якщо день народження випадає на неділю, додається 1 день
                        birthday = birthday + timedelta(days=1)
                    congratulation_list.append({'name': self.data[v].name.value, 'congratulation_day': birthday.strftime(
                        "%d.%m.%Y")})  # додавання іменинника до списку
    ...
    while True:
        ...
        if command in ["close", "exit"]:
            ...
            if len(args) != 2:
                ...
                if type(record) == str:
                    record = Record(name)
                    print(my_book.add_record(record))
